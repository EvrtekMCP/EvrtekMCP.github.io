# CREDITS

CROSSWIRE ships **no third-party code, no image files and no audio files**.
Every picture is drawn on the canvas at run time and every sound — the
effects, the callouts and the three scores — is synthesised with the
browser's built-in Web Audio API. Nothing is downloaded, hotlinked or sampled.
The type is whatever monospace the device already has.

Design: Evrtek's own concept and ninety-three rulings after playing it.
Build: CLU, with Opus agents on the mechanics and Tron on patrol.

Reference material consulted (techniques studied, no code or assets copied):

- **web.dev "A tale of two clocks"** — the lookahead audio-scheduling pattern
  the sequencer uses.
- **Mulberry32** (Tommy Ettinger, public domain) — the seeded random number
  generator behind the piece feed, so the same seed deals the same game.

Trademark note: this is an original wiring-and-colour puzzle. It is not
affiliated with, sponsored by, or endorsed by any publisher of any game it may
remind you of.

© Evrtek. All original code.
